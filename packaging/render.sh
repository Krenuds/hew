#!/usr/bin/env bash
# Renders a release's version and checksums into every packaging file.
#
#   packaging/render.sh v1.1.0
#
# Reads the asset digests from the GitHub API (no download), then rewrites
# the AUR PKGBUILD/.SRCINFO, the Gentoo ebuild filename and Manifest, and
# the Flatpak manifest/metainfo in place. Gentoo's Manifest wants BLAKE2B
# and SHA512 as well, which the API does not carry, so the two .deb files
# ARE downloaded for that step only.
set -euo pipefail
tag="${1:?usage: render.sh vX.Y.Z}"
version="${tag#v}"
repo="hew3d/hew"
...
sed -i.bak -E \
  ...
  "$here/aur/hew-bin/PKGBUILD"
sed -i.bak -E \
  -e "s/^(\tpkgver = ).*/\1${version}/" \
  -e "s#^(\tsource_x86_64 = ).*#\1https://github.com/${repo}/releases/download/${tag}/${amd64_deb}#" \
  -e "s#^(\tsource_aarch64 = ).*#\1https://github.com/${repo}/releases/download/${tag}/${arm64_deb}#" \
  -e "s/^(\tsha256sums_x86_64 = ).*/\1${sha_amd64}/" \
  -e "s/^(\tsha256sums_aarch64 = ).*/\1${sha_arm64}/" \
  "$here/aur/hew-bin/.SRCINFO"
