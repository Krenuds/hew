...
// The boxes in this repro overlap by design (that is the union under test).
// With the standing-solid gate dropped, overlapping regions extrude directly
// — interpenetration is allowed everywhere in Hew — so each box is just
// extruded at its real position.
