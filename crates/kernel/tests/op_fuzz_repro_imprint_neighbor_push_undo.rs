//! Deterministic repro distilled from `op_fuzz.rs`: imprint a sub-face on a
//! split piece, push a NEIGHBORING coplanar piece (coplanar-aware machinery
//! moves shared boundary geometry), dissolve the imprint, then undo — the
//! recorded inverse (`SplitFaceInner` re-imprinting the captured loop) is
//! refused with `LoopNotStrictlyInside`.
...
#[test]
fn imprint_neighbor_push_merge_history_unwinds() {
    let d = 0.5;
    let v = vec![
    ...
    let op = imprint_op(&object, 1815147360634333537, 0.3);
    let _ = history.apply(&mut object, op);
