...
describe('toolRegistry', () => {
  ...
  it('leaves Protractor/Slice/Edit Vertex/sketch verbs/Drawing Axes/Text/Position Texture/camera tools off the rail (no group)', () => {
    const ungrouped = TOOL_REGISTRY.filter((t) => t.group === undefined).map((t) => t.name)
    expect(ungrouped).toEqual([
      'Protractor', 'Slice', 'Edit Vertex', 'Fillet', 'Extend', 'Mirror', 'Drawing Axes', 'Text', 'Position Texture',
