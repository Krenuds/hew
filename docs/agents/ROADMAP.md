...
- Drawn in every headless output: `hew.view.line_drawing` and
  `hew.print.pdf` letter dimensions and leader text with real type, and
  `hew.view.snapshot` draws them as line work from a single-stroke font,
  since a PNG carries no text of its own. All three take the unit format
  the caller names (HEW_API.md §7.2)
